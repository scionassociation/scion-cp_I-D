...
- To pass an SCMP error message to the upper-layer process, implementations extract the upper-layer protocol type from the quoted offending packet in the body of the SCMP error message and used to select the appropriate process to handle the error.  In case of an unknown error type, implementations should assume a SCMP header length of 8 bytes, verify that the subsequent bytes represent a SCION header, and attempt to extract the offending packet.
