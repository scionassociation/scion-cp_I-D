...
As described previously, the goal of SCION’s beaconing process is to discover paths between any two ASes. The path-registration and -lookup mechanisms provide endpoints with access to these paths. An on-path adversary can, for example, interfere with endpoint communication by not disseminating beacons or simply dropping packets. An adversary can also attempt to attract traffic by selectively disseminating PCBs or by forging new PCBs. This section examines several possible approaches to manipulating paths in the SCION control plane, and shows for each case to what extent SCION's design can prevent the corresponding attack or help to mitigate it.
...
- By intercepting and disseminating a PCB on its way from AS A to neighbor AS B, and injecting its (AS M's) own hop fields into the PCB toward downstream ASes. This could offer B an attractive up-segment traversing AS M to the core.
