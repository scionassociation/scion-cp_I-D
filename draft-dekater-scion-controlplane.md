...
The following code block is the main PCB definition in Protobuf message format.
...
In Protobuf message format, extensions are specified as follows:
...
1. First, the source endpoint queries the control service in its own AS (i.e., the source AS) for the required segments (SegmentsRequest). The control service has up-path segments stored in its path database. Additionally, the control service checks if it has appropriate core- and down-path segments in store as well; in this case it returns them immediately.
...
[](#app-c) illustrates how the lookup of path segments in SCION works.
