...
4. Endpoint implementations may use the original packet in the body of the SCMP error message to pass the SCMP error message to the upper-layer process. To identify the process, the implementation can assume a length of 8 bytes for header of the SCMP error, then verify that the subsequent bytes represent a valid SCION header, and then extract the network port of the originating process from the upper-layer protocol. In case the extraction is not successful, the SCMP message MUST be silently dropped.
