...
Implementations MUST respect the following rules when processing SCMP messages:
  ...
  (a) If an SCMP error message of unknown type is received at its destination, it MUST be passed to the upper-layer process that originated the packet that caused the error, if it can be identified, see (d).
  (b) If an SCMP informational message of unknown type is received, it MUST be silently dropped.
  (c) Every SCMP error message MUST include as much of the offending SCION packet as possible. The error message packet - including the SCION header and all extension headers MUST NOT exceed **1232 bytes** in order to fit into the minimum MTU (see {{I-D.dekater-scion-dataplane}} section "Deployment Considerations/MTU").
  (d) Endpoint implementations may use the original packet in the body of the SCMP error message to pass the SCMP error message to the upper-layer process. To identify the process, the implementation can assume a length of 8 bytes for header of the SCMP error, then verify that the subsequent bytes represent a valid SCION header, and then extract the network port of the originating process from the upper-layer protocol. In case the extraction is not successful, the SCMP message MUST be silently dropped.
  (e) An SCMP error message MUST NOT be originated in response to any of the following:
